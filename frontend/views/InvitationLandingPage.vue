<!-- src/views/InvitationLanding.vue -->
<template>
    ...
    <div class="max-w-md w-full space-y-8">
      <div>
        ...
        <p class="mt-2 text-center text-sm text-gray-600">
          You've been invited to join our LLMAIx-v2 platform. Please verify your invitation to create an account.
        </p>
      </div>
      ...
      <div v-else-if="error" class="bg-white shadow rounded-lg p-8 text-center">
        ...
        <h3 class="mt-4 text-lg font-medium text-red-800">Invalid invitation</h3>
...
async function validateInvitation() {
  try {
    ...
    if (!response.data) {
      ...
      // If your backend returns the email associated with the invitation
...
function goToRegister() {
  // Navigate to register page with the token
  router.push({
    name: 'Register',
    query: {
      token: token.value,
