<template>
  <teleport to="body">
    <transition name="fade">
      <div v-if="isModal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4" @click="handleBackdropClick">
        <div class="absolute inset-0 bg-black/50 backdrop-blur-sm"></div>
        <div class="relative bg-white rounded-3xl shadow-2xl max-w-8xl w-full max-h-[95vh] flex flex-col ring-1 ring-blue-100 overflow-hidden" @click.stop>
          <!-- Header -->
          <div class="flex items-center justify-between gap-4 px-8 py-6 border-b rounded-t-3xl bg-white/90">
            ...
            <button @click="$emit('close')" class="text-gray-400 hover:text-blue-700 hover:bg-blue-50 rounded-full p-2 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500" aria-label="Close" autofocus>
          ...
          <div class="flex-1 overflow-y-auto p-8 bg-white/70">
            ...
            <div v-else-if="error" class="bg-red-50 border-l-4 border-red-400 p-5 mb-5 rounded-lg flex items-start gap-2">
              <svg class="h-6 w-6 mt-1 text-red-400" fill="none" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.7 7.3a1 1 0 00-1.4 1.4L8.6 10l-1.3 1.3a1 1 0 101.4 1.4L10 11.4l1.3 1.3a1 1 0 001.4-1.4L11.4 10l1.3-1.3a1 1 0 00-1.4-1.4L10 8.6 8.7 7.3z" clip-rule="evenodd"/></svg>
            ...
            <template v-else-if="trial">
                ...
                <div class="flex flex-col md:flex-row md:justify-between gap-6">
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-1">
                      <h2 class="text-xl font-bold text-blue-900 truncate">{{ trial.name || `Trial #${trial.id}` }}</h2>
                      <span v-if="trial.status" class="ml-2 px-2 py-0.5 rounded-full text-xs font-semibold shadow"
                        :class="{
                    ...
                    <div v-if="trial.description" class="text-gray-700 text-sm mb-1">{{ trial.description }}</div>
                    <div class="flex flex-wrap gap-x-6 gap-y-1 text-sm text-gray-600 mt-1">
                      <span><span class="font-semibold">Started:</span> {{ formatDate(trial.created_at, true) }}</span>
                      <span><span class="font-semibold">Model:</span> {{ trial.llm_model }}</span>
                      <span v-if="trial.prompt"><span class="font-semibold">Prompt:</span> <span class="text-gray-800">{{ trial.prompt.name || '[unnamed prompt]' }}</span></span>
                      <span v-if="trial.document_set"><span class="font-semibold">Document Set:</span> <span class="text-gray-800">{{ trial.document_set.name || ('Set #' + trial.document_set.id) }}</span></span>
                      <span><span class="font-semibold">Documents:</span> {{ trial.document_ids?.length || 0 }}</span>
                    </div>
                  </div>
                  <div class="flex flex-col items-start md:items-end gap-2 min-w-[200px]">
                    ...
                    <span v-if="totalUsage.total_tokens !== undefined" class="text-xs bg-blue-100 px-3 py-1 rounded-lg font-semibold text-blue-800 mt-1" title="Sum of prompt and completion tokens across all results">
                      Usage: {{ totalUsage.prompt_tokens || 0 }} prompt / {{ totalUsage.completion_tokens || 0 }} completion / <b>{{ totalUsage.total_tokens || 0 }}</b> total tokens
                    </span>
                    <div v-if="trial.advanced_options && Object.keys(trial.advanced_options).length" class="mt-2 bg-white rounded-lg border border-blue-100 px-4 py-2 shadow text-xs max-w-xs">
                      <div class="text-xs font-semibold text-blue-700 mb-1">LLM Advanced Options</div>
                      <ul>
                        <li v-for="(value, key) in trial.advanced_options" :key="key" class="flex items-center gap-1 mb-0.5">
              ...
              <div v-if="trial?.meta?.failures && Object.keys(trial.meta.failures).length" class="bg-red-50 border border-red-200 rounded-xl p-5 mb-6">
                <div class="flex items-center gap-2 mb-2">
                  ...
                  <span class="font-semibold text-red-700">{{ Object.keys(trial.meta.failures).length }} document error{{ Object.keys(trial.meta.failures).length === 1 ? '' : 's' }}</span>
                </div>
                <details class="mt-1">
                  ...
                  <ul class="list-disc list-inside mt-2 text-sm text-red-800">
                    <li v-for="(err, docId) in trial.meta.failures" :key="docId"><span class="font-semibold">Doc {{ docId }}:</span> {{ err }}</li>
                  </ul>
                </details>
              </div>
              ...
              <div v-if="!trial.results || trial.results.length === 0" class="flex flex-col items-center justify-center py-16 bg-gray-50 rounded-lg border border-gray-200">
                <svg class="h-14 w-14 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
              ...
              <div v-else class="grid grid-cols-1 gap-5">
                <div v-for="(res, index) in trial.results" :key="index" class="bg-white shadow border border-gray-100 rounded-xl transition-shadow hover:shadow-lg">
                  <!-- Row header -->
                  <div @click="toggleResultExpansion(index)" class="cursor-pointer flex items-center justify-between px-6 py-4 border-b hover:bg-gray-50/70 transition-colors rounded-t-xl select-none">
                    <div class="flex flex-col gap-0.5">
                      <div class="flex items-center flex-wrap gap-2">
                        <span class="w-7 h-7 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-base font-bold">{{ index + 1 }}</span>
                        <span class="font-medium text-gray-800">{{ documentLabels[index]?.name || 'Loading document name...' }}</span>

                        <!-- Status pills -->
                        <span v-if="res.result" class="text-[10px] uppercase tracking-wide bg-green-100 text-green-700 px-2 py-0.5 rounded">OK</span>
                        <span v-else class="text-[10px] uppercase tracking-wide bg-red-100 text-red-700 px-2 py-0.5 rounded">Error</span>

                        <span v-if="res.additional_content?.finish_reason && res.additional_content.finish_reason !== 'stop'"
                        ...
                        <span v-if="res.additional_content?.truncation_analysis?.likely_truncated"
                      ...
                      <span v-if="documentLabels[index]?.original && documentLabels[index]?.original !== documentLabels[index]?.name"
                            class="text-xs text-gray-400 italic ml-10 truncate max-w-xs">(Original: {{ documentLabels[index].original }})</span>
                    </div>
                    <svg class="w-5 h-5 text-gray-400 transition-transform duration-200" :class="{ 'rotate-180': expandedResults[index] }" viewBox="0 0 20 20" fill="currentColor">
                  ...
                  <div v-if="expandedResults[index]" class="p-6 bg-gradient-to-b from-white to-blue-50/20">
                    <!-- Inline error panel -->
                    <div v-if="!res.result || res.additional_content?.json_error" class="mb-4 bg-red-50 border border-red-200 text-red-800 text-sm rounded-lg p-4">
                      ...
                      <details v-if="res.additional_content?.tuning_advice" class="mt-2">
                        ...
                        <ul class="list-disc list-inside mt-1">
                          <li v-for="(rec, i) in res.additional_content.tuning_advice.recommendations" :key="i">
                            ...
                            <span v-if="rec.suggested_value"> → <code>{{ rec.suggested_value }}</code></span>
                    ...
                    <div class="flex gap-6" :class="viewMode[index] === 'vertical' ? 'flex-col' : 'flex-col md:flex-row'">
                      ...
                      <div v-if="showDocumentPanel[index]" class="bg-gray-50 p-5 rounded-xl overflow-auto flex-1 max-h-[480px] border border-gray-100">
                        <h4 class="text-sm font-semibold mb-3 text-gray-700 flex items-center gap-1.5">
                          <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3M3 17V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2z" /></svg>
                          Original Document
                        </h4>
                    ...
                    <div v-if="getReasoningContent(index) || getAdditionalContent(index)?.usage || getAdditionalContent(index)?.finish_reason" class="mt-6">
                      <button @click="showReasoningPanel[index] = !showReasoningPanel[index]" class="px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg text-sm flex items-center gap-1.5 transition-colors duration-150 shadow-sm">
                        <svg :class="showReasoningPanel[index] ? 'rotate-90' : ''" class="h-4 w-4 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
                        <span>{{ showReasoningPanel[index] ? 'Hide' : 'Show' }} LLM Reasoning & Metadata</span>
                      </button>
                      <div v-if="showReasoningPanel[index]" class="bg-blue-50/60 border border-blue-100 rounded-lg mt-3 p-5">
                        ...
                        <div v-if="getAdditionalContent(index)?.usage" class="mb-2">
                          ...
                          <ul class="text-xs text-blue-900 ml-2">
                            <li v-for="(v, k) in getAdditionalContent(index).usage" :key="k"><span class="font-medium">{{ k.replace(/_/g, ' ') }}:</span> {{ v }}</li>
                          </ul>
                        </div>
                    ...
                    <div class="mt-6 flex flex-wrap gap-3 justify-end">
                      <button @click="toggleViewMode(index)" class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 rounded-lg text-sm flex items-center gap-1.5 transition-colors duration-150 shadow-sm">
                      ...
                      <button @click="toggleDocumentPanel(index)" class="px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg text-sm flex items-center gap-1.5 transition-colors duration-150 shadow-sm">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          ...
                          <path v-else d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268-2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                        </svg>
            ...
            <div v-else class="flex flex-col items-center justify-center py-16">
              <svg class="h-14 w-14 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
              <span class="text-gray-500 mt-3">Trial not found</span>
              <button @click="$emit('close')" class="mt-6 inline-block px-4 py-2 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors duration-150 shadow">Return to trials</button>
            </div>
          </div>
        </div>
      </div>
...
const renderMarkdown = (text) => { try { return marked(text); } catch { return text; } };
const isMarkdown = (text) => {
  if (!text) return false;
  try { return text.includes('#') || text.includes('**') || text.includes('*') || text.includes('[') || text.includes('```') || /\n\s*-\s/.test(text) || /\n\s*\d+\.\s/.test(text); }
  catch { return false; }
};

// Helpers to parse/inspect additional_content
const parseAdditional = (ac) => { if (!ac) return null; if (typeof ac === 'string') { try { return JSON.parse(ac); } catch { return null; } } return ac; };
...
const toggleDocumentPanel = async (i) => {
  ...
  if (showDocumentPanel.value[i] && !documentPdfUrls.value[i] && !documentPdfLoading.value[i]) {
    ...
    try {
      ...
      const d = r.data;
      let fileId;
      if (d.preprocessed_file_id && d.preprocessed_file?.file_type === 'application/pdf') fileId = d.preprocessed_file_id;
      else if (['application/pdf', 'image/png', 'image/jpeg'].includes(d.original_file?.file_type)) fileId = d.original_file_id;
      if (fileId) {
        const fr = await api.get(`/project/${props.projectId}/file/${fileId}/content?preview=true`, { responseType: 'blob' });
        const blob = new Blob([fr.data], { type: fr.headers['content-type'] });
        documentPdfUrls.value[i] = URL.createObjectURL(blob);
      }
    } catch (err) { console.error(err); toast.error("Failed to load document"); }
    finally { documentPdfLoading.value[i] = false; }
  }
};
...
<style>
.markdown-content { font-size: 0.875rem; line-height: 1.5; color: #333; }
.markdown-content h1, .markdown-content h2, .markdown-content h3,
.markdown-content h4, .markdown-content h5, .markdown-content h6 { margin-top: 1.5em; margin-bottom: 0.5em; font-weight: 600; line-height: 1.25; }
.markdown-content h1 { font-size: 1.5em; } .markdown-content h2 { font-size: 1.25em; } .markdown-content h3 { font-size: 1.125em; }
.markdown-content p { margin-bottom: 1em; }
.markdown-content ul, .markdown-content ol { margin-bottom: 1em; padding-left: 1.5em; }
.markdown-content li { margin-bottom: 0.25em; }
.markdown-content code { background-color: #f0f0f0; padding: 0.2em 0.4em; border-radius: 3px; font-size: 85%; font-family: SFMono-Regular, Consolas, Menlo, monospace; }
.markdown-content pre { background-color: #f6f8fa; border-radius: 3px; padding: 1em; overflow: auto; margin-bottom: 1em; }
.markdown-content pre code { background-color: transparent; padding: 0; }
.markdown-content a { color: #0366d6; text-decoration: none; } .markdown-content a:hover { text-decoration: underline; }
</style>
