      ...
      <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full border p-6 flex flex-col">
        ...
        <div class="mb-4">
          ...
          <select v-model="format" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
            <option value="json">JSON (per-document)</option>
        ...
        <div class="mb-4">
          ...
          <label class="flex items-center text-sm">
            ...
            />
            <span class="ml-2">Include document content</span>
          </label>
        </div>
...
async function download() {
  ...
  try {
    ...
    link.href = url;
    link.setAttribute('download', `trial_${props.trial.id}_results.${format.value}`);
