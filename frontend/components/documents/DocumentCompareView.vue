  ...
  <div class="flex h-full">
    ...
    <template v-if="hasDisplayableOriginalFile">
      ...
      <div class="w-1/2 border-r overflow-auto p-4 flex flex-col">
        <h4 class="font-medium text-slate-700 mb-2">Original File</h4>
        <!-- PDF Viewer -->
        <iframe
        ...
        <div
          ...
          class="flex-1 flex items-center justify-center bg-slate-100 rounded-lg p-4"
        >
          <img
        ...
        <div v-else class="text-slate-400 flex items-center justify-center flex-1">
          No original file available
        </div>
      </div>
      <!-- Right: Extracted Text -->
      <div class="w-1/2 overflow-auto p-4 flex flex-col">
        <h4 class="font-medium text-slate-700 mb-2">Extracted Text</h4>
        <div class="prose max-w-none bg-slate-50 p-2 rounded-lg flex-1" v-html="safeMarkdown" />
      </div>
    </template>
    <!-- No displayable original file: show only extracted text (full width) -->
    <div v-else class="w-full overflow-auto p-4 flex flex-col">
      <h4 class="font-medium text-slate-700 mb-2">Extracted Text</h4>
      <div class="prose max-w-none bg-slate-50 p-4 rounded-lg" v-html="safeMarkdown" />
    </div>
  </div>
</template>
