  ...
  >
    <div class="p-4">
      ...
      <div class="flex items-start justify-between mb-3">
        <div class="flex items-center space-x-3">
          <div class="flex-shrink-0">
            ...
            <div v-else-if="task.status === 'completed'" :class="['h-8 w-8 rounded-full flex items-center justify-center', hasWarnings ? 'bg-amber-100' : 'bg-green-100']">
              <svg class="h-5 w-5" :class="hasWarnings ? 'text-amber-600' : 'text-green-600'" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path v-if="!hasWarnings" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                <path v-else stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-5.355-7.695a9 9 0 1110.71 0 9 9 0 01-10.71 0z" />
              </svg>
            </div>
      ...
      <div v-if="task.status === 'completed'" class="flex flex-wrap gap-4 items-center text-sm mt-2 mb-1">
        ...
        <span
          v-if="hasWarnings"
          class="flex items-center text-amber-600 font-medium"
          :title="`${warningCount} file(s) have warnings`"
        >
          <svg class="h-4 w-4 inline mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const hasWarnings = computed(() => {
  ...
  return fileTasks.some(ft => ft.warnings && (ft.warnings.messages || ft.warnings.skipped_rows));
});
