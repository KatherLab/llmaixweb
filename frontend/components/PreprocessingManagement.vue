  ...
  <div class="p-6 space-y-6">
    ...
    <div v-if="completedTasks.length > 0" class="bg-white rounded-2xl shadow-lg border border-gray-200 p-0 mt-6">
      ...
      <transition appear mode="out-in" name="fade-expand">
        <div v-show="showCompletedTasks" key="completed-tasks-list" class="px-4 pb-2 pt-1 space-y-1 overflow-hidden">
          <div
          ...
          >
            <div class="flex items-center gap-3 min-w-0">
              ...
              >
                <svg v-if="task.status === 'completed' && !task.file_tasks?.some(ft => ft.warnings?.messages || ft.warnings?.skipped_rows)" class="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div class="flex flex-col text-xs min-w-0">
                <div class="flex items-center gap-2">
                  ...
                  <span v-if="task.status === 'failed'" class="ml-2 px-1.5 py-0.5 rounded bg-red-100 text-red-600 text-2xs font-semibold uppercase tracking-wide">FAILED</span>
                  <span v-else-if="task.status === 'cancelled'" class="ml-2 px-1.5 py-0.5 rounded bg-yellow-100 text-yellow-700 text-2xs font-semibold uppercase tracking-wide">CANCELLED</span>
                  <span v-else-if="task.status === 'errored'" class="ml-2 px-1.5 py-0.5 rounded bg-yellow-100 text-yellow-700 text-2xs font-semibold uppercase tracking-wide">ERROR</span>
                  <span v-else-if="task.status === 'completed'" class="ml-2 px-1.5 py-0.5 rounded bg-green-100 text-green-700 text-2xs font-semibold uppercase tracking-wide">COMPLETED</span>
                </div>
                <div class="flex items-center gap-3 mt-0.5">
                  <span v-if="task.failed_files > 0" class="text-red-500">✗ {{ task.failed_files }} failed</span>
                  <span v-if="task.processed_files - task.failed_files - (task.skipped_files || 0) > 0" class="text-green-500">✓ {{ task.processed_files - task.failed_files - (task.skipped_files || 0) }} succeeded</span>
                  <span v-if="task.skipped_files > 0" class="text-yellow-600">⚠ {{ task.skipped_files }} skipped</span>
                  <span v-if="task.file_tasks?.some(ft => ft.warnings?.messages || ft.warnings?.skipped_rows)" class="text-amber-600">⚠ {{ task.file_tasks.filter(ft => ft.warnings?.messages || ft.warnings?.skipped_rows).length }} with warnings</span>
                  <span v-if="task.status === 'failed' && task.message" class="text-red-400 truncate max-w-[200px]">{{ task.message }}</span>
                </div>
              </div>
            </div>
            <div class="flex items-center gap-2 shrink-0">
              <span :title="task.completed_at || task.updated_at" class="text-xs text-gray-400">{{ formatRelativeTime(task.completed_at || task.updated_at) }}</span>
            </div>
          </div>
          <div v-if="completedTasks.length > 5" class="text-center pt-2">
            <button class="text-sm text-emerald-700 hover:text-emerald-900 font-medium inline-flex items-center gap-1" @click.stop="showAllCompleted = !showAllCompleted">
              <span v-if="!showAllCompleted">Show {{ completedTasks.length - 5 }} more</span>
              <span v-else>Show less</span>
              <svg :class="['h-4 w-4 transition-transform duration-200', showAllCompleted ? 'rotate-180' : '']" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path d="M19 9l-7 7-7-7" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
              </svg>
            </button>
          </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
      <div class="p-6">
        ...
        <!-- Engine Selection -->
        <div class="mb-6">
          <label class="block text-sm font-medium text-gray-700 mb-3">OCR / Text Extraction Engine</label>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            >
              <svg :class="selectedEngine === 'ocrmypdf' ? 'text-blue-600' : 'text-gray-400'"
              ...
              <span class="text-xs text-gray-500 mt-1 text-center">{{ getEngineSubtitle('ocrmypdf') }}<br/>No configuration needed</span>
            </button>
            ...
            >
              <svg :class="selectedEngine === 'mistral_ocr' ? 'text-blue-600' : 'text-gray-400'"
                   class="h-8 w-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <span class="text-xs text-gray-500 mt-1 text-center">{{ mistralOcrEnabled ? getEngineSubtitle('mistral_ocr') : 'Disabled by server' }}</span>
            </button>
            ...
            >
              <svg :class="selectedEngine === 'llm_vision' ? 'text-blue-600' : 'text-gray-400'"
                   class="h-8 w-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <span class="text-xs text-gray-500 mt-1 text-center">{{ visionOcrEnabled ? getEngineSubtitle('llm_vision') : 'Disabled by server' }}</span>
            </button>
          </div>
        </div>

        <!-- Engine-specific settings -->
        <div class="bg-gradient-to-br from-blue-50/60 to-white border border-blue-200 rounded-2xl px-8 py-6 space-y-4 mt-4 mb-4">
          <!-- Force OCR (global) -->
          ...
          <!-- Mistral settings -->
          <div v-if="selectedEngine === 'mistral_ocr'" class="space-y-3 border-t border-blue-100 pt-4">
            <!-- Always visible: prompt area (empty for Mistral) -->
            <p class="text-sm text-gray-600">Base URL: <code class="text-xs bg-blue-50 px-1.5 py-0.5 rounded">{{ serverDefaults.mistral_api_base }}</code></p>
            ...
            <div>
              ...
              >
                <svg :class="showMistralAdvanced ? 'rotate-90' : ''" class="h-3 w-3 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div v-if="showMistralAdvanced" class="space-y-3 pl-3 border-l-2 border-blue-100">
              <div>
                ...
                <input v-model="mistralApiKey" type="text" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" placeholder="Enter API key (leave empty to use server-configured key)" autocomplete="off" />
              </div>
              <div>
                <label class="block text-xs font-medium text-gray-700 mb-1">Model (optional)</label>
                <input v-model="mistralModel" type="text" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" placeholder="mistral-ocr-latest" />
              </div>
            </div>
          </div>

          <!-- Vision LLM settings -->
          <div v-if="selectedEngine === 'llm_vision'" class="space-y-3 border-t border-blue-100 pt-4">
            <!-- Always visible -->
            <div>
              <label class="block text-xs font-medium text-gray-700 mb-1">Prompt (optional)</label>
              <textarea v-model="visionPrompt" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" :placeholder="serverDefaults.vision_ocr_prompt" rows="2" />
            </div>
            ...
            <div>
              ...
              >
                <svg :class="showVisionAdvanced ? 'rotate-90' : ''" class="h-3 w-3 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path d="M9 5l7 7-7 7" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
                </svg>
                Advanced
              </button>
            ...
            <div v-if="showVisionAdvanced" class="space-y-3 pl-3 border-l-2 border-blue-100">
              <div>
                <label class="block text-xs font-medium text-gray-700 mb-1">API Key</label>
                <input v-model="visionApiKey" type="text" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" placeholder="Enter API key (leave empty to use server-configured key)" autocomplete="off" />
              </div>
              <div>
                <label class="block text-xs font-medium text-gray-700 mb-1">Base URL</label>
                <input v-model="visionBaseUrl" type="text" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" :placeholder="serverDefaults.vision_ocr_api_base || 'https://api.openai.com/v1'" />
              </div>
              <div>
                <label class="block text-xs font-medium text-gray-700 mb-1">Model</label>
                <input v-model="visionModel" type="text" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" :placeholder="serverDefaults.vision_ocr_model" />
              </div>
              <div>
                <label class="block text-xs font-medium text-gray-700 mb-1">Max Image Dimension (pixels)</label>
                <input v-model.number="visionMaxImageDim" type="number" min="400" max="4096" step="100" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" />
                <p class="mt-1 text-xs text-gray-500">Images will be scaled to this max dimension before sending to the vision model.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="mb-6">
          <div class="flex justify-between items-center mb-3">
            ...
            <div class="text-sm text-gray-500">{{ selectedFiles.length }} of {{ availableFiles.length }} selected</div>
          </div>
          <FileSelector
...
const visionPrompt = ref('Extract all text from this image and return it as clean markdown. Preserve the original structure, headings, lists, and formatting as much as possible.');
...
const fetchOcrSettings = async () => {
  try {
    ...
    if (r.data.mistral_ocr_enabled !== undefined) mistralOcrEnabled.value = r.data.mistral_ocr_enabled;
...
const setupPolling = () => {
  ...
  const poll = () => {
    const active = allTasks.value.filter(t => ['pending', 'processing', 'in_progress'].includes(t.status));
...
const startPreprocessing = async () => {
  ...
  try {
    const inlineConfig = {
      name: selectedEngine.value === 'ocrmypdf' ? 'Quick Process' : `Custom ${selectedEngine.value}`,
...
// Dialogs
const showCancelTaskDialog = (task) => { cancelTaskPending.value = task; cancelDeleteMode.value = false; showCancelDialog.value = true; };
function handleCancelDialogSecondary() {
  if (cancelDeleteMode.value) { showCancelDialog.value = false; cancelTaskPending.value = null; cancelDeleteMode.value = false; }
  else cancelDeleteMode.value = true;
}
const doCancelTask = async (task, keepProcessed) => {
  showCancelDialog.value = false; cancelDeleteMode.value = false;
  if (!task) return;
  try {
  ...
  finally { cancelTaskPending.value = null; }
};
const cancelTask = async (task) => {
  if (!confirm('Keep already processed files?')) return;
  try { await api.post(`/project/${props.projectId}/preprocess/${task.id}/cancel?keep_processed=true`); toast.success('Task cancelled'); fetchPreprocessingTasks(); }
  catch (e) { toast.error('Failed to cancel task'); }
};
...
const retryTask = async (task) => {
  try { const r = await api.get(`/project/${props.projectId}/preprocess/${task.id}/retry-failed`); allTasks.value.unshift(r.data); toast.success('Retry task created'); setupPolling(); }
  catch (e) { toast.error('Failed to retry'); }
};
const viewTaskDetails = async (task) => {
  try { const r = await api.get(`/project/${props.projectId}/preprocess/${task.id}`); selectedTask.value = r.data; }
  catch (e) { selectedTask.value = task; }
};
const retryFailedFiles = async (taskId) => {
  try { const r = await api.get(`/project/${props.projectId}/preprocess/${taskId}/retry-failed`); allTasks.value.unshift(r.data); toast.success('Retry task created'); selectedTask.value = null; setupPolling(); }
  catch (e) { toast.error('Failed to create retry task'); }
};
const selectAllFiles = () => { selectedFiles.value = availableFiles.value.map(f => f.id); };
const resetForm = () => {
  selectedFiles.value = [];
...
watch(() => props.files, (f) => { availableFiles.value = f; }, {immediate: true});
watch(() => activeTasks.value.length, (l) => { if (l > 0 && !pollInterval) setupPolling(); });
watch(() => completedTasks.value.length, () => { showAllCompleted.value = false; });
</script>
