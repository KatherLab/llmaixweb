    ...
    <transition name="fade">
      ...
      >
        ...
        >
          <!-- Header -->
          <div
            class="px-8 py-6 bg-gray-50 border-b rounded-t-2xl flex items-center justify-between"
          >
          ...
          <div class="flex-1 overflow-y-auto px-8 py-6">
                ...
                <tbody>
                  <tr v-for="(row, ridx) in preview.rows" :key="ridx">
                    <td v-for="(cell, cidx) in row" :key="cidx" class="px-2 py-1">
                      <span v-if="cell === '' || cell == null" class="text-gray-300 italic"
                        >empty</span
                      >
            ...
            <div class="space-y-6">
              ...
              <div
                v-if="preprocessingStrategy === 'row_by_row' && headerLabels.length"
                class="space-y-3"
              >
                <!-- Text columns -->
                <div>
          ...
          <div class="px-8 py-6 border-t flex justify-end gap-3 bg-gray-50 rounded-b-2xl">
            <button
              class="px-4 py-2 rounded text-gray-600 bg-white border border-gray-200 hover:bg-gray-100"
              @click="tryClose"
            >
              Cancel
            </button>
            <button
              :disabled="
                saving || (preprocessingStrategy === 'row_by_row' && textColumns.length === 0)
              "
          ...
          >
            <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md border text-center">
              ...
              <div class="text-gray-600 mb-6">
                Your changes to import configuration will be lost.
              </div>
