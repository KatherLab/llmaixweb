...
function typeOf(def: SchemaProperty | undefined): string {
 ...
 */
function walk(
  def: SchemaDefinition | SchemaProperty | undefined,
  prefix: string,
  depth: number,
  ...
  const props = (def.properties as Record<string, SchemaProperty>) || {}
  ...
  for (const [key, child] of Object.entries(props)) {
    ...
    const title = child && (child.title as string | undefined)
    ...
    if (type === 'object' && child?.properties) {
      ...
      fields.push({
        ...
        required: false,
        format: (child.items?.format as string) || '',
...
export function flattenSchemaFields(
  schemaDefinition: SchemaDefinition | string | null | undefined,
): SchemaField[] {
  let schema: unknown = schemaDefinition
  ...
  return walk(schema as SchemaDefinition, '', 0, [])
}
