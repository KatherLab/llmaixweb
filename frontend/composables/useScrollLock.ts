 ...
 */
import { watch, onMounted, onUnmounted, type WatchSource } from 'vue'

// Module-level lock counter shared across all useScrollLock consumers.
let lockCount = 0

interface UseScrollLockOptions {
  /** Auto-lock as soon as the component mounts (and unlock on unmount). */
  autoLock?: boolean
  /** A reactive ref or getter to watch — locks while truthy, unlocks while falsy. */
  watch?: WatchSource<boolean> | null
}
...
export function useScrollLock(options: UseScrollLockOptions = {}): {
  ...
  if (watchRef !== null) {
    ...
    const readWatch = (): boolean =>
      typeof watchRef === 'function' ? (watchRef as () => boolean)() : watchRef.value
