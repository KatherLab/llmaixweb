...
import type { DocumentFilter, DocumentListItem } from '@/types'

/** Options accepted by `buildDocQueryParams` / `fetchAllDocumentIds`. */
interface DocQueryOptions {
  limit?: number
  offset?: number
  search?: string
...
/** Options object passed to `useDocumentPagination`. */
...
/** Return type of `useDocumentPagination`. */
interface UseDocumentPaginationReturn {
  docsPage: Ref<DocumentListItem[]>
  totalDocs: Ref<number>
  pageSize: Ref<number>
  page: Ref<number>
  isLoadingDocs: Ref<boolean>
  docsError: Ref<string | null>
  documentLookup: Ref<Map<number, string>>
  buildDocQueryParams: (opts?: DocQueryOptions) => DocumentFilter
  fetchDocuments: (opts?: { reset?: boolean }) => Promise<void>
  fetchAllDocumentIds: (q?: DocQueryOptions) => Promise<number[]>
  getDocLabel: (docId: number) => string
  toISODateStart: (d: string) => string | undefined
  toISODateEndExclusive: (d: string) => string | undefined
}

export function useDocumentPagination(
  options: UseDocumentPaginationOptions = {} as UseDocumentPaginationOptions,
  ...
  // Current page items
  const docsPage = ref<DocumentListItem[]>([])
  ...
  const documentLookup = ref(new Map<number, string>())

  const toISODateStart = (d: string): string | undefined =>
    d ? new Date(`${d}T00:00:00.000Z`).toISOString() : undefined
  const toISODateEndExclusive = (d: string): string | undefined =>
    d ? new Date(`${d}T23:59:59.999Z`).toISOString() : undefined

  const buildDocQueryParams = (opts: DocQueryOptions = {}): DocumentFilter => {
  ...
  const fetchDocuments = async ({ reset = false } = {}): Promise<void> => {
  ...
  const fetchAllDocumentIds = async (q: DocQueryOptions = {}): Promise<number[]> => {
    const ids: number[] = []
    let offset = 0
    // API max
    const limit = 500
    ...
    try {
      ...
      const pushIds = (items: DocumentListItem[]): void => {
  ...
  const getDocLabel = (docId: number): string =>
    documentLookup.value.get(docId) || `Document #${docId}`
